...
public class AnimationUtils
{
	...
	public static <T extends Entity> AnimatedEntityModel getModelForEntity(T entity)
	{
		...
		if (entityRenderer instanceof RenderLiving)
		{
			...
			ModelBase entityModel = renderer.mainModel;
			if (entityModel instanceof AnimatedEntityModel)
			{
				return (AnimatedEntityModel) entityModel;
			}
			else {
				GeckoLib.LOGGER.error("Model for " + entity.getCommandSenderName() + " is not an AnimatedEntityModel. Please inherit the proper class.");
				return null;
			}
		}
		else {
			GeckoLib.LOGGER.error("Could not find valid renderer for " + entity.getCommandSenderName());
			return null;
		}
	}
	 ...
	 * @return
	 */
	public static <T extends Entity> Render getRenderer(T entity)
	{
